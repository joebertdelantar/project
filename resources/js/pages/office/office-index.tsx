...
interface OfficeIndexProps {
    offices: OfficePagination;
    filters: FilterProps;
}

interface OfficePagination {
    data: Office[];
    links: LinkProps[];
    from: number;
    to: number;
...
export default function OfficeIndex({ offices, filters }: OfficeIndexProps) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="relative flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                    ...
                    {data.search && (
                        <button
                            ...
                            onClick={() => {
                                ...
                                router.get(route('offices.index'), {}, { preserveState: true, preserveScroll: true }); // Reset the search results
                            }}
                        >
                            <X size={20} />
                        </button>
                ...
                <div className="flex-1 overflow-hidden rounded-xl bg-white p-4 shadow-md">
                    <h2 className="mb-4 text-lg font-semibold">Office List</h2>
                    <table className="w-full divide-gray-200">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            ...
                            {/* Map through offices and display them in the table */}
                            {offices.data.map((office: any) => (
                                <tr key={office.id}>
                                    <td className="px-6 py-4 text-sm whitespace-nowrap text-gray-900">{office.office_acronym}</td>
                                    <td className="px-6 py-4 text-sm whitespace-nowrap text-gray-900">{office.office_name}</td>
                ...
                {/* Global Pagination */}
                <Pagination links={offices.links} from={offices.from} to={offices.to} total={offices.total} />
            </div>
        </AppLayout>
    );
}
